...
function buildPages() {
  pages = [];
  // Cover page
  const toc = D.summaries.map((s) => `<li><span>${escapeHtml(s.topic)}</span><span>${s.count}</span></li>`).join("");
  pages.push(`
    ...
    <div class="stat-row">
      ...
      <div class="stat"><b>${D.summaries.length}</b><span>topics</span></div>
    </div>
    <h2>In this issue</h2>
    <ul class="toc">${toc}</ul>
  ...
  for (let i = 0; i < D.summaries.length; i += 2) {
    const chunk = D.summaries.slice(i, i + 2);
...
function newsletterMarkdown() {
  ...
  md += `**Fetched:** ${kpi.fetched} · **AI stories:** ${kpi.kept} · **Topics:** ${D.summaries.length}\n\n---\n\n`;
  for (const s of D.summaries) {
